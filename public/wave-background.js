class AWaves extends HTMLElement {
   ...
   */
  connectedCallback() {
    // Check if Noise is available, if not wait for it
    if (typeof window.Noise === 'undefined') {
      // Wait for Noise to be available
      const checkNoise = () => {
        if (typeof window.Noise !== 'undefined') {
          this.initializeWaves();
        } else {
          setTimeout(checkNoise, 100);
        }
      };
      checkNoise();
    } else {
  ...
  /**
   * Initialize waves after Noise is available
   */
  initializeWaves() {
    // Elements
    ...
    this.lines = []
    this.paths = []
    this.noise = new window.Noise(Math.random())

// services/price-alert/watcher.ts
...
// import { sdk } from "../../src/lib/somnia"; // Removed static import
...
async function main() {
    try {
        const { sdk } = await import("../../src/lib/somnia");
        ...
        setInterval(async () => {
            try {
                ...
                try {
                    ...
                    console.log(`📊 Found ${activeThresholds.length} VALID price threshold(s)`);
        ...
        }, 15000); // check every 15 seconds (faster, but watch out for rate limits)
